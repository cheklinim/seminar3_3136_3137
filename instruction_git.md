...
*При первом запуске Git Hub нужно "подружить" Git и Git Hub. В большинстве случаев авторизация проходит автоматически, с подтверждением авторизации на сайте. После этого локальный репо привызявается к хабу.*
...
### Изменение данных в чужих репозиториях

У любого публичного репозотория можно посмотреть и скачать код проета. Внести изменения в чужой репозиторий без специального разрешения нельзя, но можно предложить владельцу репозитория свою версию проекта. Для этого необходимо:

1. Зайти в интересующий репозиторий на Git Hub и нажать кнопку "Folk". После этого репо скопируется на хабе с пометкой, что он зафолчен у другого пользователя.
1. Делаем "clone" своей копии репо.
1. **Создаем новую ветку** и в неё вносим все необходимые изменения. Сливать ветки НЕ нужно.
1. Фиксируем все коммиты.
1. Отправляем свою версию на Git Hub, с помощью команды "push".
1. При обновлении на сайте Git Hub появится кнопка "Compare & Pull Request", при нажатии на неё, изменения будут предложены автору проекта. 
