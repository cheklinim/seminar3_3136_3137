...
Внеси в VSCode некоторые измнения в скопированную папку. Сохрани, добавь фиксацию и коммит. При этом все изменения произойдут только локально. Для того, что передать их в удалённый репозиторий, набери:

    git push

## Приём данных

Если ты вносил изменения прямо на сервисе gitHub, то возникнет обратная ситуация - все изменения произойдут в удалённом репозитоии, а чтобы принять их локально, сделай так:

    git pull

При этом git автоматически создаёт новую ветку и через merge добавляет полученные данные тебе в VSCode.

## Вилка

Нет, речь не еде. Вилка - это образное представление множественного клонирования одного и того же удалённого репозитоиия. Если тебе требуется внести предложения в другой публичный проект с gitHub, воспльзуйся Fork'ом. У тебя на gitHub появится весь проект.

Перейди в VSCode и обязательно создай отдельную ветку, переключись на неё.

Сформируй своё предложение, отправь данные, снова перейди на gitHub и воспользуйся кнопкой:

    pull request

## Заключение

На этом наше путешествие в мир git окончено. Надеюсь, у тебя всё получилось! Практикуйся чаще, чтобы запомнить все описанные команды.

Кстати, есть классные тренажёр по git, набери в любом поисковике: 
