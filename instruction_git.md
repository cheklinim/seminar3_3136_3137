...
Для предложение владельцу репозитория наших правок необходимо:
1. Изменения в локальном репозитории загружаем на наш удаленный репозиторий с помощью команды *git push*. 
2. В учетной записи на удаленном репозитории появляется кнопка *Compare & pull request*. Нажав ее необходимо заполнить появившиеся поля. 

Если кнопка не появилась, можно отправить запрос через кнопку *Contribute*.

3. После выполнения вышеуказанных действий, владелец изначального удалённого репозитория получит запрос на добавление наших изменений.
