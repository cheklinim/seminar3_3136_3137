...
Мы не влезаем в чужую ветку master, поэтому создаем новую ветку, например:

git branch remoted

Переключаемся на новую ветку и можем работать с ней

 ### Отправить изменения автору

 Для того, чтобы отправить изменения автору, мы воспользуемся командой 

    pull request

Для этого в GitHub сделать любой из двух шагов (одинаковы):
