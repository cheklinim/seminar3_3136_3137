...
### **Порядок действий для внесения изменений в чужой репозиторий**

Для предложения своих изменений в чужой репозиторий необходимо выполнить следующие действия:

1. Сделать **"fork"** репозитория, находящегося на Github (на странице репозитория), таким образом отобразив этот репозиторий у себя в аккаунте.
2. С помощью команды **"git clone url"** перенести удаленный репозиторий в локальный. (URL можно посмотреть во вкладке code репозитория на GH)
3. С помощью команды **"cd folder name"** поменять деректорию работы git на нужную нам.
4. Создать новую ветку и произвести желаемые изменения.
5. Использовать команду **"git push"** для отправки изменений в удаленный репозиторий (в свою копию (fork)).
6. Создать запрос на внесение изменений в основном репозитории с помощью **"pull request"** На сайте GitHub.
