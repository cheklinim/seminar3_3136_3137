...
После создания удаленного репозитория мы возвращаемся в терминал и прописываем следующие команды для связывания локального репозитория и удаленного

    git remote add origin https://github.com/Padatoo/test.git
    git branch -M master
    git push -u origin master

Нужно будет ввести логин и пароль аккаунта в Github. 
...
используется для выгрузки содержимого локального репозитория в удаленный репозиторий. дополнительные ключи не нужны если репозитории уже связаны и ветка настроена.
...
форк это копия (обычно чужого) репозитория с которой можно работать точно так же как и со своей, не меняя оригинальный репозиторий.
...
это запрос на вливание изменений из вашей ветки (которую вы получили через форк) в основную ветку исходного репозитория. Таким образом они попадут к хозяевам проекта.

Перед тем как запросить пулл реквест нужно соблюсти ряд правил работы с опенсорсом, таких использование своей собственной ветки и такого как придерживание общего стиля проекта.