...
Если нам необходимо доработать/внести изменения/предложить свои дополнения в чужой удаленный репозиторий для этого:
* заходим на страницу пользователя
* заходим в нужный репозиторий
* нажимаем кнопку fork, таким образом копируем репозиторий к себе.

И далее уже работаем с ним, т.е. с копией, которая храниться в нашем новом репозитории.

## Скачивание удаленного репозитория в локальный

Для того, чтобы скачать удаленный репозиторий необходимо:
* создать на своем компьютере новую папку, в которую будет скачан реопзиторий
* заходим в эту папку в VSC
* копируем ссылку на удаленный репозиторий на github
* копируем весь репозиторий в нашу папку с помощью команды:

    git clone _link_ 

* не забываем перейти в новую папку, котрая появиться после копирования с помощью комнады

    cd _name_folder_

## Отправка доработки/изменений/предложений в чужой репозиторий

Необходимо с помощью команды git push отправить все изменения с свой репозиторий, созданный с помощью fork. Далее с помощью pull request предложить свои изменения автору исходного репозитория.